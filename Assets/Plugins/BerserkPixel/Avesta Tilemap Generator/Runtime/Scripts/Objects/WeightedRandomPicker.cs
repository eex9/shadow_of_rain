...
namespace BerserkPixel.Tilemap_Generator
{
    ...
    {
        private readonly IList<T> candidates;
        private double totalWeight;
        private readonly IList<float> weights;
        ...
        public WeightedRandomPicker(IList<T> candidates, IList<float> weights)
        {
            if (candidates.Count != weights.Count)
                throw new MissingMemberException(
                    $"Candidate count {candidates.Count} is not equal to weight count {weights.Count}. Abort.");
